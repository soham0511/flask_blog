...
@app.route("/")
def home():
    flash("Please join the community by contributing.","success")
...
def edit(sno):
    if ('user' in session and session['user']==params['admin_user']):
        if request.method=='POST':
            ...
            date=datetime.now()
            # if sno=0 then new post else edit post
            if sno=='0':
...
def contact():
    if(request.method=='POST'):
        ...
        #                   body = message + "\n" + phone
        #                   )
        flash("Details submitted successfully. We will get back to you soon.","success")
